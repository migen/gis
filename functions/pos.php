	...
	foreach($positems AS $row){		
		...
		if($row['io']==1){ 
			...
			foreach($combor AS $cid){
				$q .= "UPDATE {$dbo}.`03_products` SET level = level-".$qty.",`t{$tml}` = `t{$tml}` -".$qty." WHERE `id`='".$cid."' LIMIT 1;";
			}
		}	/* io */	
		// pr($q);exit;
	} 	/* positems */
...
function getOrigPosdetails($db,$rxref_posid,$dbg=PDBG){		
	$dbo=PDBO;	
	$q="SELECT datetime FROM {$dbo}.`30_pos` WHERE id='$rxref_posid' LIMIT 1; ";
	$sth = $db->querysoc($q);
	$data['pos']=$sth->fetch();
...
function getPosdetails($db,$posid,$dbg=PDBG){		
	$dbo=PDBO;	
	$q="SELECT datetime FROM {$dbo}.`30_pos` WHERE id='$posid' LIMIT 1; ";
	$sth = $db->querysoc($q);
	$data['pos']=$sth->fetch();
	$q = "SELECT 
			pd.id AS pdid,pd.*,p.name AS product,p.combo,p.barcode,p.code
		FROM {$dbo}.`30_positems` AS pd
			LEFT JOIN {$dbo}.`03_products` AS p ON pd.product_id = p.id
		WHERE pd.`pos_id` = '$posid' ORDER BY p.name LIMIT 100; ";
	$sth = $db->querysoc($q);
	$data['positems']=$sth->fetchAll();
	return $data;	
