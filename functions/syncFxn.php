...
function syncTsum($db,$sy,$exe=false){
...
function syncSummaries($db,$sy){
	$dbo=PDBO;$dbg=VCPREFIX.$sy.US.DBG;
	$q1=" SELECT `id` AS `scid` FROM {$dbo}.`00_contacts` WHERE `role_id`=1 ORDER BY id; ";
	$sth=$db->querysoc($q1);
	$a=$sth->fetchAll();
	$ac=$sth->rowCount();
	$ar=buildArray($a,'scid');	
...
function syncSummext($db,$sy){
	...
	/* 2 */			
	$q2=" SELECT scid FROM {$dbg}.`05_summext` ORDER BY scid; ";			
	$sth=$db->querysoc($q2);
	$b=$sth->fetchAll();
	$bc=$sth->rowCount();
	$br=buildArray($b,'scid');
	...
	if(!empty($ix)){
		...
		foreach($ix AS $scid){ $q .= " ($scid),"; } $q = rtrim($q,",");$q .= "; ";
		pr($q);
		$sth=$db->query($q);echo "All R1 Contacts to Summext: ";echo ($sth)? "Success":"Fail";			
	}	/* has-ix */
...
function syncProfiles($db,$sy){
	$dbo=PDBO;$dbg=VCPREFIX.$sy.US.DBG;
	$q1=" SELECT `id` AS `scid` FROM {$dbo}.`00_contacts` ORDER BY id; ";
	$sth=$db->querysoc($q1);
	$a=$sth->fetchAll();
	$ac=$sth->rowCount();
	$ar=buildArray($a,'scid');	
...
function syncTables($db,$sy){
	...
	syncSummext($db,$sy);	/* contacts-summaries */
