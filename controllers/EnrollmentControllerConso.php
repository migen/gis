...
if(isset($_GET['filter'])){
	...
	$q="SELECT p.scid,p.date,c.name AS student,p.amount AS amount,p.orno,
			l.name AS level,s.name AS section,pm.name AS paymode,c.code AS studcode,c.sy,c.sy_registered ";
...
/* 
		$sibsarr=scidSiblings($db,$scid,$sy);			
		$data['siblings']=$sibsarr['rows'];		
		$data['siblings_count']=$sibsarr['count'];		
		$data['family']=$sibsarr['family'];		
		$data['family_id']=$sibsarr['family']['family_id'];		
 */
...
public function paydates($params=NULL){	
	...
	$data['sy']=$sy=isset($params[0])? $params[0]:$_SESSION['year'];
	$q="SELECT * FROM {$dbo}.03_paydates WHERE sy=$sy ORDER BY position;";
	$sth=$db->querysoc($q);
	$data['rows']=$sth->fetchAll();
	$data['count']=$sth->rowCount();
...
public function level($params=NULL){
	...
	$data['ensy']=$data['sy']=$sy=$_SESSION['settings']['sy_enrollment'];
	...
	if($lvl){
		$db=&$this->baseModel->db;$dbo=PDBO;$dbg=VCPREFIX.$sy.US.DBG;
		$cond=isset($_GET['all'])? NULL:"tsum.enrolled_balance<=0 AND";
		...
		$q="SELECT summ.scid,c.sy,c.code,c.name,m.name AS paymode,tsum.*,cr.name AS classroom
			...
			WHERE $cond cr.level_id=$lvl AND cr.num=$num
		...
		$q="SELECT summ.scid,c.sy,c.code,c.name,m.name AS paymode,tsum.*,cr.name AS classroom
			...
			WHERE $cond t.sy=$sy AND cr.level_id=$lvl
			ORDER BY cr.num,c.name;";						
		debug($q);
		$sth=$db->querysoc($q);
		$data['rows']=$sth->fetchAll();
...
public function conso($params=NULL){
