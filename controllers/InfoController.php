...
public function __construct(){
	...
	$acl = array(array(5,0),array(4,0),array(9,0));		/* RMIS,RACAD,RREG */
...
require_once(SITE."functions/info.php");
$db =& $this->model->db;
$dbo=PDBO;$dbg=PDBG;
$data['id']=$id=isset($params[0])? $params[0]:false;
if($id){
	$data['row']=readInfo($db,$id);
}	/* id */

$this->view->render($data,'info/view');
...
if(isset($_POST['submit'])){
	...
	foreach($posts AS $post){ 
		if(!empty($post['info'])){ $db->add("{$dbo}.info",$post); }
	}
	$id=lastId($db,"{$dbo}.info");	
	$url="info/view/$id";
	flashRedirect($url,'New info added.');
}	/* post */

$data['infotypes']=fetchRows($db,"{$dbo}.infotypes","*","id");
$this->view->render($data,'info/add');
...
$this->view->render($data,'info/modify');
...
$this->view->render($data,"info/index");
...
public function edit($params){		
	...
	$this->view->render($data,'info/edit');		
