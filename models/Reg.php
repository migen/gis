...
public function units($crid,$dbg=PDBG){
	$q = " SELECT c.id AS scid,c.code AS student_code,c.name AS student,c.is_active,
			sum.units_previous,sum.units_current,sum.units_total,
			s.level_entry
	...
	$cr = $_SESSION['classroom'] = $data['classroom'] = $this->getClassroomDetails($crid);
	return $data;

}	/* fxn */

 
public function profiles($crid){
	$dbo=PDBO;	
	$q = " SELECT 
			c.id AS contact_id,c.code AS student_code,c.name AS student,c.is_active,
	...
	$sth = $this->db->querysoc($q);
	$data['students'] = $sth->fetchAll(); 	
	$data['classroom'] = $this->getClassroomDetails($crid);
...
public function getProfile($contact_id,$dbg=PDBG){
	$dbo=PDBO;	
	$q = " 
	SELECT c.id AS contact_id,c.code AS student_code,c.account,c.name AS student,c.`sy`,c.`sy` AS year,
...
$cond = (!is_null($ctype))? ' AND sub.crstype_id = '.$ctype : NULL;
	$q = " SELECT 
		...
		WHERE cr.level_id = $level_id	$cond; ";
	$sth = $this->db->querysoc($q);
	return $sth->fetchAll();
...
public function getCourses($crid,$dbg=PDBG){	
	...
	$q = "SELECT 
		...
		FROM {$dbg}.05_courses AS crs
			INNER JOIN {$dbg}.05_classrooms AS cr ON crs.crid = cr.id
			INNER JOIN {$dbo}.`05_subjects` AS sub ON crs.subject_id = sub.id
		WHERE crs.crid = $crid AND crs.crstype_id = 1				
...
public function readStudent($id,$dbg=PDBG){	
	...
	$q = "SELECT 
				...
				s.is_sectioned,c.`sy`,c.crid AS crid,c.is_male,
				cr.acid AS acid
			FROM {$dbg}.05_students AS s
...
public function levelyear($dbg,$sy,$lvlid,$male=2,$order="c.name",$fields=NULL,$filter=NULL,$limits=NULL,$active=0){
	...
	$q = "
		SELECT 		
			sum.*,sum.`id` AS sumid,
			cr.`acid` AS acid,cr.`name` AS classroom,
			c.`is_male`,$fields
			c.`id` AS scid,c.`code` AS student_code,c.`name` AS student,
			s.`contact_id`,sum.`crid` AS crid,s.`prevcrid` AS prevcrid,c.`sy` AS sy	
		...
		WHERE cr.level_id = '$lvlid' $condactive $is_male $filter
		ORDER BY $order 
		$limited ; ";
