...
<p><?php  include_once('incs/ornos_filter.php'); ?></p>
...
<tr>
	...
	<td><?php echo ($rows[$i]['is_void']==1)? 'Y':'N'; ?></td>
	<td><?php echo $rows[$i]['orno']; ?></td>
	<td><?php echo $rows[$i]['remarks']; ?></td>
	<td>
		<a href="<?php echo URL.'invoices/printorno/'.$rows[$i]['orno']; ?>" >Print</a>
		| <a href="<?php echo URL.'ornos/edit/'.$rows[$i]['id']; ?>" >View</a>
		| <a href="<?php echo URL.'ornos/edit/'.$rows[$i]['id']; ?>" >Edit</a>
	    | <a href="<?php echo URL.'ornos/delete/'.$rows[$i]['id']; ?>" onclick="return confirm('Sure?');" >Del</a>	
		<?php if($rows[$i]['is_void']==1): ?>
			<input id="xbtn<?php echo $i; ?>"  type="button" 
				onclick="xunvoidOrno(<?php echo $rows[$i]['id'].','.$i; ?>);" value="UnVoid" />
		<?php else: ?>
			<input id="xbtn<?php echo $i; ?>"  type="button" 
				onclick="xvoidOrno(<?php echo $rows[$i]['id'].','.$i; ?>);" value="Void" />		
		<?php endif; ?>
	</td>
</tr>
<?php endfor; ?>
