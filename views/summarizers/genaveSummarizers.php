...
<h5>
	STD Summarizer - General Average (<?=$num_students;?>)
	| <a href="<?php echo URL; ?>">Home</a> 
	<?php echo isset($_SERVER['HTTP_REFERER'])? '| <a href="'.$_SERVER['HTTP_REFERER'].'" >Back</a>' : ''; ?>			
	| <a href='<?php echo URL."matrix/grades/$crid/$sy/$sqtr"; ?>'>Matrix</a> 	
	| <a href='<?php echo URL."spiral/crid/$crid/$sy/$sqtr"; ?>'>Spiral</a> 	
...
<?php endif; ?>
	| <a href='<?php echo URL."genave/truval/$crid/$sy/$sqtr"; ?>'>Truval</a> 	

	
<?php $sems = array(0,1,2); ?>
	| <select id="semval" class="vc50" onchange="jsredirect('summarizers/genave/'+crid+ds+sy+ds+qtr+'?sem='+this.value);" >
		<?php foreach($sems AS $sv): ?>
			<option <?php echo ($sem==$sv)? 'selected':NULL; ?> ><?php echo $sv; ?></option>
			<?php endforeach; ?>
		</select>
...
<?php if($num_subjects==0){ echo "<h5 class='brown' >*NO subjects, try Selecting a Semester.</h5>"; } ?>

<?php if($num_subjects>0): ?>

<p>1) &deciave=<?php echo $deciave; ?> <br />


</p>
<p>Please follow the sequence 1) Summarize  2) Ranking </p>
<?php $pcid=$_SESSION['user']['parent_id']; ?>
...
<table class='gis-table-bordered table-fx'>
	...
	<tr class="hd" ><th class='white bg-blue2' >Locking</th> 
		<th>
			...
			<?php endif; ?>				
	| 		
	<a href="<?php echo URL.'averages/reave/'.$crid.DS.$sy.DS.$qtr.'&deciave='.$deciave; ?>" >
		Re-Average</a>			
			
		</th>
	</tr>	
	<tr><th class='white headrow'>Adviser</th><td class="vc500" ><?php echo $cr['adviser'].' #'.$cr['acid']; ?></td></tr>
	<tr><th class='white headrow'>Classroom</th><td><?php echo $cr['level'].'-'.$cr['section'].' #'.$cr['crid']; ?>
		| Status-Q<?php echo $qtr; ?>: <?php echo ($is_locked)? 'Closed':'Open';  ?>
		</td></tr>
	<tr><th class='white headrow'>Dates</th><td><?php 
		echo 'Summarized: '.date('M-d, Y D',strtotime($classroom['finalized_date_q'.$qtr])); 
		echo ' | Printed: '.date('M-d, Y D',strtotime($_SESSION['today'])); 
	?>
	</td></tr> 
...
<tr class='bg-blue2'>
	...
	<?php foreach($subjects AS $row): ?>
		<th class="center <?php echo ($row['is_finalized_q'.$qtr]==0)? 'bg-red':NULL; ?>" >
			...
			<?php echo ($row['units']!=1)? '<br />U:'.$row['units']:NULL; ?>
		</th>	
	<?php endforeach; ?>
	<th class="center" >AG</th>
	<th class="center" >TG</th>
	<th class="hd" >SCID</th>
	<th class="hd center" ></th>
</tr>
	...
	<?php $qqtr = 'q'.$j; ?>
		...
		<?php for($i=0;$i<$num_subjects;$i++): ?>		<!-- subject iterator per row -->
				<td  class='colshading' > <?php 
							...
							$rdscore = number_format($score,$decicard); 	
							echo $rdscore; 	
							${'q'.$j.'t'} += $rdscore;
					?>
				</td>								
		<?php endfor; ?>
			...
			<?php 
				@${'q'.$j.'t'} = ${'q'.$j.'t'}/$num_subjects; $rdtg = number_format(${'q'.$j.'t'},$decigenave);
				$rdbga=number_format($students[$g]['ave_q'.$j],$decigenave);
				$same=($rdbga==$rdtg)? true:false;
			?>
				
			<td class="final <?php echo (!$same)? 'bg-salmon':null; ?>">
				<input class="vc50 center" name="sum[<?php echo $g; ?>][ave_q<?php echo $j; ?>]" 
					value="<?php echo $rdtg; ?>"  readonly />
				<!-- add q1t to fgt -->
				<?php if(!$sem): ?>
					<?php $ftg += $rdtg; ?>				
				<?php else: ?>
				...
				<?php if($cr['is_k12']): ?>
				<?php 
					$rdga=number_format(${'q'.$j.'t'},$decigenave);
					$rfg=rating($rdga,$ratings);
				?>
				<?php $intype = ($is_k12)? 'text' : 'hidden'; ?>
				<input class="vc25 center" type="<?php echo $intype; ?>" 
					name="sum[<?php echo $g; ?>][ave_dg<?php echo $j; ?>]"  value="<?php echo $rfg; ?>" readonly />			
				<?php endif; ?>
				
				<?php 
					if($rdbga!=$rdtg){ echo "not updated"; $updated=false; }
				?>
			...
			<!--  num_diff -->		
			<?php if($students[$g]['ave_q'.$j] != $rdtg){ $num_diff++; } ?>					
...
<!-- ============================== for FG ROW not column ============================== -->
	...
	<?php for($i=0;$i<$num_subjects;$i++): ?> <!-- subject iterator -->			
			<?php 				
				$score=isset($row[$i]['q'.$intfqtr])? $row[$i]['q'.$intfqtr]:0;
				$rdscore = number_format($score,$deciave);
				$total_fg+=$rdscore;
								
				
			?>
			<td class='colshading' > <?php echo number_format($score,$deciave); ?></td>
	<?php endfor; ?>		<!-- subject iterator -->
			...
			<?php 
				...
				$rdftg=number_format($ftg,$decifgenave);
				$rdfg=number_format($students[$g]['ave_q'.$intfqtr],$decifgenave);
				$same=($rdftg==$rdfg)? true:false;
	...
	<input class="vc50 center" name="sum[<?php echo $g; ?>][ave]" 
		value="<?php echo $rdftg; ?>"  readonly />
				<?php if($cr['is_k12']): ?>
					<?php $rftg = rating($rdftg,$ratings); ?>
			...
			<input type="hidden" name="sum[<?php echo $g; ?>][scid]" value="<?php echo $students[$g]['scid']; ?>" >
	</tr>	
...
<script>
	...
	$(function(){		
		...
		hd();
		columnHighlighting();			
		if(!updated){ alert('Click Summarize to update.'); }
	}) 
