...
<h5>
	Summarizer - General Average (<?=$num_students;?>)
...
?>
	...
	<?php $qqtr = 'q'.$j; ?>
		...
		<?php endfor; ?>
			...
			<?php 
				@${'q'.$j.'t'} = ${'q'.$j.'t'}/$total_units; $rdtg = number_format(${'q'.$j.'t'},$decigenave); 
...
<!-- ============================== for FG ROW not column ============================== -->
	...
	<?php for($i=0;$i<$num_subjects;$i++): ?> <!-- subject iterator -->			
			<?php 				
				...
				$units=$row[$i]['units'];				
				$uscore = number_format($score,$deciave)*$units ;
				$total_fg+=number_format($uscore,$deciave);
	...
	<td class="<?php echo ($same)? NULL:'bg-red'; ?>" > 	
				<?php 
					// echo "total_fg $total_fg <br />";
					// echo "total_units $total_units <br />";
				?>	
