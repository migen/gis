...
<h5 class="screen" >
	School Class Index Reports (CIR-<?php echo $count; ?>)
...
<tr class="headrow" >
	...
	<th>Honors</th>
	<th>Submissions</th>
	<th class="center" >Summarizer<br />Step 1</th>
	<th class="center" >Ranking<br />Step 2</th>
	<th class="center" >Report<br />Step 3</th>
	<th>ID</th>	
</tr>
...
<tr>
	...
	<td><?php echo ($classrooms[$i]['is_finalized_q'.$qtr]==1)? 'Y':'-'; ?></td>
<td><a href='<?php echo URL."classlists/classroom/".$classrooms[$i]['id']."/$sy?sch=".$_GET['sch']; ?>' >List</a></td>
<td><a href='<?php echo URL."attendance/monthly/".$classrooms[$i]['id']."/$sy/$qtr"; ?>' >Q<?php echo $qtr; ?></a></td>
<td>
	<a href='<?php echo URL."matrix/grades/".$classrooms[$i]['id']."/$sy/$qtr?sch=".$_GET['sch']; ?>' >Matrix</a>
	<?php if(!empty($classrooms[$i]['conduct_id'])): ?>
		| <a href='<?php echo URL."conducts/records/".$classrooms[$i]['conduct_id']."/$sy/$qtr?sch=".$_GET['sch']; ?>' >Cond</a>
	<?php else: ?>			
