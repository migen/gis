...
/* @gcontroller-mcr */
	...
	$get=isset($_GET)? sages($_GET):NULL;
...
<span class="screen" >		
	...
	<?php if($classroom['level_id']>13): ?>
		...
		| <a href='<?php echo URL."mcr/view/$crid/$sy/$qtr?$get&sem=1"; ?>' >Sem1</a>
		| <a href='<?php echo URL."mcr/view/$crid/$sy/$qtr?$get&sem=2"; ?>' >Sem2</a>			
		
		| <a href='<?php echo URL."mcr/view/".$data['classroom']['id']."/$sy/5?sem=1&deci=2"; ?>' />Q5</a>	
		| <a href='<?php echo URL."mcr/view/".$data['classroom']['id']."/$sy/6?sem=2&deci=2"; ?>' />Q6</a>				
	<?php endif; ?>
...
<?php foreach($data['grades'] AS $row): ?>
	...
	<?php $iqtr = (($sem) || ($param_qtr==6))? 6:5; ?>	
	<?php for($j=2;$j<=$iqtr;$j++): ?>
	<?php $qtr = 'q'.$j; ?>
		<tr>
			<td>&nbsp;</td> <td>&nbsp;</td>
			<td class="colshading" ><?php echo ($j==5)? 'FG':'Q'.$j; ?></td>
		<?php for($i=0;$i<$num_subjects;$i++): ?>		
				<td class="colshading center" ><?php 
