...
function setPage($cq){
	...
	switch($cq['ctype_id']){
		case 1: {
			$page['url'] = ($cq['with_scores']==1)? 'teachers/scores':'teachers/grades';
			$page['desc'] = ($cq['with_scores']==1)? 'Scores':'Grades';			
			break;		
		}		
		case 2: $page['url'] = 'cav/traits';$page['desc'] = 'Traits';break;
		case 5: $page['url'] = 'conducts/records';$page['desc'] = 'Conducts';break;
		default: $page['url'] = 'teachers/scores';$page['desc'] = 'Scores';break;
	}
	return $page;
}
...
<tr>
	...
	<td><?php echo ($cq[$i]['semester']>0)? "S".$cq[$i]['semester']." - ":NULL; echo $cq[$i]['label']; ?></td>
	<td>
	...
	<td> 
		<?php $page_array = setPage($cq[$i]);$page=$page_array['url'];$desc=$page_array['desc'];?>
		<a href='<?php echo URL."$page/".$cq[$i]['course_id']."/$ssy/1"; ?>'>Q1</a> | 
		<a href='<?php echo URL."$page/".$cq[$i]['course_id']."/$ssy/2"; ?>'>Q2</a> | 
		<a href='<?php echo URL."$page/".$cq[$i]['course_id']."/$ssy/3"; ?>'>Q3</a> | 
		<a href='<?php echo URL."$page/".$cq[$i]['course_id']."/$ssy/4"; ?>'>Q4</a> 
		- <?php echo $desc; ?>
	</td>
