...
<tr>
	...
	<td><?php echo $row['due_on']; ?>
		<input type="hidden" id="amount-<?php echo $i; ?>-1" value="<?php echo number_format($row['amount'],2); ?>" />	
		<input type="hidden" id="balance-<?php echo $i; ?>-1" value="<?php echo number_format($pbr['balance'],2); ?>" />	
...
<tr class="<?php echo ($row['amount']==0)? 'bg-pink':null; ?>" >
	<td><?php echo $i; ?></td>
...
<!-- for axnFeetype and xsetFeetype, refactor to fetch promise async await -->
...
function xsetFeetype(feetype_id){
...
function xdelete(dbtable,i,num){
	...
	if (confirm('DANGEROUS! Sure?')){
		...
		var amount=$('#amount-'+i+'-'+num).val();
		var feetype=$('#feetype-'+i+'-'+num).val();		
		var logdetails = `${studname} - ${tableName} delete at ledger of P${amount} for ${feetype}.`;		
		var pdata='task='+task+'&id='+id+'&dbtable='+dbtable+'&logdetails='+logdetails+'&logsy='+sy+'&module_id=2';
...
function axnFeetype(id,num){
	// var feetype_id = $("#feetype_id-"+num).val(id);
	$("#feetype_id-"+num).val(id);
	if(num==1){ discountPromiseOne(id); }
	xsetFeetype(id);
...
function putPayment(dbtable,num){
	...
	var logdetails=`${studname} - payment add at ledger of P${amount} for ${feetype}.`;
	...
	pdata+="&reference="+reference+"&notes="+notes+"&orno="+orno+"&ptr="+ptr+"&received="+received+"&change="+change+"&module_id=2";
	$('#btnAdd-'+num).hide();
...
function putPayable(dbtable,num){
	...
	var logdetails=`${studname} - payable add at ledger of P${amount} for ${feetype}.`;

	var pdata="task="+task+"&dbtable="+dbtable+"&logdetails="+logdetails+"&logsy="+sy+"&module_id=2";
