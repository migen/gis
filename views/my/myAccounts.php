...
<td><?php echo ($rows[$i]['id']==$rows[$i]['parent_id'])? '1':'-'; ?></td>
<td><?php echo $rows[$i]['role_id'].'-'.$rows[$i]['privilege_id']; ?></td>
<td><?php echo $rows[$i]['role']; ?></td>
<td><a href='<?php echo URL."my/switcher/".$rows[$i]['ucid']; ?>' ><?php echo $rows[$i]['account']; ?></a></td>
</tr>
<?php endfor; ?>
</table>
