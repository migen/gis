...
<?php else:	/* 2nd outcome : different criteria  */ ?>		
		...
		<?php else: ?>		
			<td colspan="2" class="colshading center bg-lightgreen" > 
			<?php $pnv = 0; ?>
			- </td>
		<?php endif; ?>
...
<th colspan="<?php echo ($showcid)? '3':'2'; ?>" > Passed <br /> <span class='red'>Failed </span></th>
	...
	?>
		...
		<?php if($activities[$i]['criteria_id'] == @$activities[$j]['criteria_id']): ?>		
			<!-- PF column -->
			<th class='center' >
				<?php $passed=$activities[$i]['passed']; echo "P: ".$passed."<br />"; ?>
				<span class='red'><?php $failed=$activities[$i]['failed']; echo "F: ".$failed; ?></span>
				<br /><span><?php $passratio=($passed/$num_students)*100; echo round($passratio).'%';  ?></span>
