...
function getLevelRanksWithConditionSjam($db,$lvl,$sy,$qtr,$cond=NULL,$limitcond=NULL){
...
$cond="AND sumx.honor_q{$qtr}>0 AND sumx.is_qualified_q{$qtr}=1";
$rows=getLevelRanksWithConditionSjam($db,$lvl,$sy,$qtr,$cond,$limitcond=NULL);
$count=count($rows);	
...
<h5>
	<?php echo $level['name']; ?> Ranks With Honors (SJAM Level Genave - <?php echo $count; ?>)
