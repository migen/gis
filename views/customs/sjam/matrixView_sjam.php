...
$link_scores=isset($_GET['scores'])? true:false;
...
<?php if(!isset($_GET['scores'])): ?>	
	| <a href='<?php echo URL."matrix/view/$crid/$sy/$qtr?scores"; ?>' >Scores-Link</a> 			
<?php endif; ?>
...
<tr class='bg-blue2'>
	...
	<?php foreach($courses AS $row): ?>	
		...
		<th class="center <?php echo ($s%2)?'bg-blue1':NULL; ?> <?php echo ($row['on_reports']==0)?'shd':NULL; ?> " >
			<?php echo $s.'<br />'; ?>
			<?php if($link_scores): ?>
				<a href="<?php echo URL.'teachers/scores/'.$row['course_id'].DS.$sy.DS.$qtr; ?>"><?php echo $row['course_code']; ?></a><br />
			<?php else: ?>
...
function redirPage(){
	var sy=$('#sy').val();
	var qtr=$('#qtr').val();
	var url=gurl+'/matrix/view/'+crid+'/'+sy+'/'+qtr;
	window.location=url;
