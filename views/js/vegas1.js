...
function jsredirect(str){ var url=gurl+'/'+str;window.location = url; }
function redirectClassroom(axn,crid){ sy=$('#sy').val();var rurl=gurl+'/'+home+'/'+axn+'/'+crid+'/'+sy;window.location = rurl;	}
function doubleConfirm(){ if (confirm('Damgerous! Are you 100% sure?')) document.forms[0].submit(); return false; }
function excel(){ $("#btnExport").click(function () {	$("#tblExport").btechco_excelexport({ containerid: "tblExport",datatype: $datatype.Table}); }); }	

function chkAllvar(x){
	$('#chkAll'+x).click(function(event) {
		if(this.checked) { $('.chk'+x).each(function() { this.checked = true; });			
		} else { $('.chk'+x).each(function() { this.checked = false; }); }
	});
}	/* fxn */


function xdelrow(dbtbl,id,i){
var vurl=gurl+'/ajax/ajax.php';var task="xdelrow";$.ajax({ url:vurl,dataType:"json",type:"POST",data:"task="+task+"&dbtbl="+dbtbl+"&id="+id,async:true,
success: function(s) { $('#trow-'+i).remove(); } }); }

function smartPaste(srcbox){
	var aim=$('#classbox').val();var str=$("#"+srcbox).val();var res=str.split("\n");
	for(var i in res){ var tmp = res[i];document.getElementById(aim+i).value = tmp; } }	

function pasteFromExcel(srcbox,aim){
	var str=$("#"+srcbox).val();var res=str.split("\n");
	for(var i in res){ var tmp = res[i];document.getElementById(aim+i).value = tmp; } }


function looper(str){ var sr=str.split(','); var url=gurl+'/';
	for(var i=0; i<sr.length;i++) { sr[i]=sr[i].replace(/^\s*/, "").replace(/\s*$/, "");url+=sr[i] +'/'; window.location=url; }

function traceAuth(){
	var ctp=$('#uhdp').val();$('#uhdp').val('');$('#hdpdiv').hide();var hp=CryptoJS.MD5(ctp);if(hdpass==hp){ $('.hd').show(); } }	

function getKey(keyStroke) {
	isNetscape=(document.layers);eventChooser=(isNetscape) ? keyStroke.which : event.keyCode;
	which = String.fromCharCode(eventChooser).toLowerCase();
	for (var i in key) if (which == i) $(key[i]).focus(); }

function accordion(){
$(".accordion tbody:not(:first)").hide(); $('.accordion th').click(
function() { $('table.accordion').children('tbody').slideUp(); $(this).parents('table.accordion').children('tbody').toggle(); }) }
...
function nextViaEnter() {
    ...
    $('input:text').bind("keydown",function(e) {
        ...
        if (e.which == 13) {
            ...
            else { $('input:text')[nextIndex - 1].blur(); $('input:submit').click(); }
        }
    });
    return false;
}
...
function xgetidByCode(dbx,tbl,code){
	var vurl = gurl+'/ajax/xgetidByCode.php';$.ajax({type:"POST",url:vurl,dataType:"json",data:"dbx="+dbx+"&tbl="+tbl+"&code="+code,
	success:function(s){ $("#display").html(s.id); } }); }	

function columnHighlighting(){ var t;
    $('.colshading').hover(function() { t = parseInt($(this).index())+1;	$('td:nth-child(' + t + ')').addClass('colshadingbg'); },
		function() { $('td:nth-child(' + t + ')').removeClass('colshadingbg'); } );	 }	/* fxn */
