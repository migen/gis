...
function xgetNamesByPart(limits=20){
	...
	$.ajax({
		...
		success: function(s) { 
			...
			if(!s) { content += '<h2 class="red" >No record found!</h2>'; }
for (var i = 0; i < cs; i++) {			
	content += '<p> <span id="'+s[i].id+'" class="txt-blue b u" onclick="redirLookup(this.id);return false;" >'+s[i].name+' - '+s[i].account+' - '+s[i].code+' #'+s[i].id+'</span></p>';
}
			$('#names').append(content).show();
			content = '';
...
function xgetStudentsByPart(limits=20){
	var part = $('#part').val();	
	var vurl = gurl+'/ajax/xlookups.php';	
	var task = "xgetStudentsByPart";	
	$.ajax({
		url: vurl,dataType: "json",type: 'POST',
		data: 'task='+task+'&part='+part+'&limits='+limits,async: true,
		success: function(s) {  // console.log(s);
			...
			if(!s) { content += '<h2 class="red" >No record found!</h2>'; }
for (var i = 0; i < cs; i++) {			
	content += '<p> <span id="'+s[i].id+'" class="txt-blue b u" onclick="redirContact(this.id);return false;" >'+s[i].name+' - '+s[i].account+' - '+s[i].code+' #'+s[i].id+'</span></p>';
}
			$('#names').append(content).show();content = '';
