...
<?php if($qtr < 5 && $is_locked): ?>	
	...
	<?php if($_SESSION['settings']['with_chinese']): ?>
		| <a href='<?php echo URL."alien/sumoRanking/".$cr['crid']."/$sy/$qtr";  ?>' > Chinese Ranking </a>
	<?php endif; ?>	
<?php endif; ?>	
...
<?php endif; ?>	
	...
	| <a href="<?php echo URL.'summarizers/genave/'.$cr['crid'].DS.$sy.DS.$qtr; ?>" >Summarizer</a>
...
<h2 class='darkgray'> Submissions <?php if($qtr < 5): ?> - Q<?php echo $qtr; ?> <?php endif; ?></h2>
...
<?php if($qtr < 5): ?> <tr><th class='white bg-blue2'>Status</th><td><?php echo 'Q'.$data['qtr'].' - '; 
	...
	<tr class="hd" ><th class='white bg-blue2' >Locking</th> 
		<th>
			<?php if($is_locked): ?>
				<a href="<?php echo URL.'finalizers/openClassroom/'.$cr['crid'].DS.$sy.DS.$qtr; ?>" > Unlock </a>
			<?php else: ?>
				<a href="<?php echo URL.'finalizers/closeClassroom/'.$cr['crid'].DS.$sy.DS.$qtr; ?>" > Lock </a>			
			<?php endif; ?>				
		</th>
	</tr>
</table>
...
<?php else: ?>
	...
	<th class="vc100" >Locker</th>
	<th class="vc100 hd" > Locking </th>
<?php endif; ?>
...
<?php for($i=0;$i<$num_parents;$i++): ?> 	<!-- loop thru number of courses -->
	...
	<?php else: ?>
		...
		<td><a href="<?php echo URL.'lockers/course/'.$parents[$i]['course_id']; ?>" >Locker</a></td>
		
		<td class="hd" > 
			<?php if($parents[$i]['is_finalized_q'.$qtr]): ?>
				<a href="<?php echo URL.'finalizers/openCourse/'.$cr['crid'].DS.$parents[$i]['course_id'].DS.$sy.DS.$qtr; ?>" > Unlock </a>
			<?php else: ?>
				<a href="<?php echo URL.'finalizers/closeCourse/'.$cr['crid'].DS.$parents[$i]['course_id'].DS.$sy.DS.$qtr; ?>" > Lock </a>			
			<?php endif; ?>
		</td>
	<?php endif; ?>	
		<td class="hd">
	...
	<?php else: ?>
		...
		<td class="hd" > 
			<?php if($children[$i]['is_finalized_q'.$qtr]): ?>
				<a href="<?php echo URL.'finalizers/openCourse/'.$cr['crid'].DS.$children[$i]['course_id'].DS.$sy.DS.$qtr; ?>" > Unlock </a>
			<?php else: ?>
				<a href="<?php echo URL.'finalizers/closeCourse/'.$cr['crid'].DS.$children[$i]['course_id'].DS.$sy.DS.$qtr; ?>" > Lock </a>			
			<?php endif; ?>
		</td>		
	<?php endif; ?>	
	...
	<?php else: ?>
		...
		<td class="hd" > 
			<?php if($conducts[$i]['is_finalized_q'.$qtr]): ?>
				<a href="<?php echo URL.'finalizers/openCourse/'.$cr['crid'].DS.$conducts[$i]['course_id'].DS.$sy.DS.$qtr; ?>" > Unlock </a>
			<?php else: ?>
				<a href="<?php echo URL.'finalizers/closeCourse/'.$cr['crid'].DS.$conducts[$i]['course_id'].DS.$sy.DS.$qtr; ?>" > Lock </a>			
			<?php endif; ?>
		</td>		
