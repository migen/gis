...
<h5>
	...
	<?php if($value<2): ?>
		...
		<?php else: ?>
		| <a href='<?php echo URL."cav/matrix/$crid/$sy/$qtr?value=1"; ?>' >DG</a>				
		<?php endif; ?>
...
<?php $d['page']="Traits Summary - SY$sy-Q$qtr"; ?>
...
<?php foreach($critypes AS $row): ?>
	<?php $numtrs=$row['count']; ?>
	<?php $critype_id=$row['critype_id']; ?>
	<th class="center" colspan="<?php echo $row['num']; ?>" ><?php echo $row['critype']; ?>
		<a href='<?php echo URL."cav/matrixByCritype/$crid/$critype_id/$sy/$qtr?value={$traits_value_dg}"; ?>' ><?php echo "({$num})"; ?></a>
	</th>
<?php endforeach; ?>
<th class="shd" ></th>
</tr>
