...
// if($lvl>12){ echo "&1st,2nd,3rd <br />"; }

if($lvl==13){ $hnr_r=array(1=>7,2=>5,3=>3);
} else { $hnr_r=array(1=>8,2=>5,3=>3); }

$hnr_r[1]=isset($_GET['1st'])? $_GET['1st']:$hnr_r[1];
$hnr_r[2]=isset($_GET['2nd'])? $_GET['2nd']:$hnr_r[2];
$hnr_r[3]=isset($_GET['3rd'])? $_GET['3rd']:$hnr_r[3];

if(isset($_GET['debug'])){ pr($hnr_r); }

?>

<table class="gis-table-bordered" >
<tr>
<th>Honors Requirements</th>
<td>1st = <?php echo $hnr_r[1]; ?> </td>
<td>2nd = <?php echo $hnr_r[2]; ?> </td>
<td>3rd = <?php echo $hnr_r[3]; ?> </td>
</tr>
</table>
...
/* 4 */
function getNumberOfCourses($db,$dbg,$crid,$cond=NULL){
	$q=" SELECT COUNT(crs.id) AS num
		FROM  {$dbg}.05_classrooms AS cr 	
			LEFT JOIN  {$dbg}.05_courses AS crs ON crs.crid=cr.id
		WHERE cr.id='$crid' AND crs.crstype_id='1' AND crs.`is_active`=1 $cond ";
...
/* 9 */
function evaluateMynumpri($is_qual,$orig_honor,$my_numpri,$numpri,$hnr_r){
	if($is_qual){
		$hnr_1=$hnr_r[1];
		$hnr_2=$hnr_r[2];
		$hnr_3=$hnr_r[3];
		if($my_numpri>=$hnr_1){ $honor=1; 
		} elseif($my_numpri>=$hnr_2){ $honor=2; 
		} elseif($my_numpri>=$hnr_3){ $honor=3; 
		} else { $honor=0; }		
...
/* 4 */
$numsub=getNumberOfCourses($db,$dbg,$crid,$where="AND crs.supsubject_id>0");
$numpri=getNumberOfCourses($db,$dbg,$crid,$where="AND crs.supsubject_id<1 AND crs.is_primary=1");
...
<?php if($lvl>12): ?>	
	...
	<?php endif; ?>	
<?php endif; ?>	
	| <a href="<?php echo URL.'honors/process/'.$crid.'&1st=5&2nd=4&3rd=3'; ?>" >Customize</a>

<?php if($lvl>13): ?>
	<?php if($qtr<5): ?>
		| <a href="<?php echo URL.'honors/process/'.$crid.DS.$sy.DS.'5?sem=1'; ?>" >Sem1</a>
		| <a href="<?php echo URL.'honors/process/'.$crid.DS.$sy.DS.'6?sem=2'; ?>" >Sem2</a>
	<?php else: ?>
		<?php $currsem=($_SESSION['qtr']<3)? 1:2; ?>
...
<tr>
<td><?php echo $i+1; ?></td>
<td class="hd" ><?php $scid=$students[$i]['scid']; echo $scid; ?></td>
...
<?php 	
	$orig_honor=$honor;
	$honor=evaluateMynumpri($is_qual,$honor,$my_numpri,$numpri,$hnr_r);
...
<table class="gis-table-bordered table-altrow" style="font-size:0.8em;" >
	...
	<tr><td>1.1) Genave 90 above. (Summarizer) </td></tr>
	<tr><td>1.2) Genave 93 above (1st), 90 - 92 (2nd), 87-89 (3rd)</td></tr>
	...
	<tr><td>
		4) All Primary Subject (is_primary) grade over 90 (1st) <?php echo "Num primary: $numpri"; ?><br />
 		> At least <?php echo ($hnr_r[2])?> (2nd)<br />
 		> At least <?php echo ($hnr_r[3])?> (3rd)<br />
	</td>
</tr>
