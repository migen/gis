...
function getCutoffArray($dept=NULL){	/* sub=subcrs,min=anycrs */
	$cutarr['1st']=93;$cutarr['2nd']=90;$cutarr['3rd']=87;$cutarr['conduct']=90;$cutarr['pri']=90;$cutarr['sub']=85;$cutarr['min']=85;		
	return $cutarr;
}	/* fxn */

/* 2 */
function getHonorsStudentsLevel($db,$dbg,$lvl,$qtr,$fields="c.id",$cond,$order="genave DESC"){	
	$dbo=PDBO;
	...
	$q="SELECT $fields,summ.scid,c.code AS studcode,c.name AS student,
		...
		WHERE cr.level_id='$lvl' $cond ORDER BY $order;";
	pr($q);
	debug($q,"getLevelHonorsStudents");
	$sth=$db->querysoc($q);
	return $sth->fetchAll();
}	/* fxn */

/* 3 */
function getHonorsSubjectsLevel($db,$dbg,$lvl,$fields="crs.id",$filter=NULL,$sem=0){
$dbo=PDBO;	
...
$q = "SELECT $fields,crs.id AS crs,crs.name AS course
	...
	WHERE cr.level_id='$lvl' AND crs.crstype_id='".CTYPEACAD."' AND crs.`is_active`=1 $filter $condsem
...
/* 4 */
function getNumberOfCoursesLevel($db,$dbg,$lvl,$cond=NULL,$sem=0){
	$condsem=($sem>0)? "AND (crs.semester=$sem || crs.semester=0 )":NULL;
	$q=" SELECT COUNT(crs.id) AS num
		FROM  {$dbg}.05_classrooms AS cr 	
			LEFT JOIN  {$dbg}.05_courses AS crs ON crs.crid=cr.id
		WHERE cr.level_id='$lvl' AND crs.crstype_id='1' AND crs.`is_active`=1 $cond $condsem ";
...
/* 5 */
function honorsGradesLevel($db,$dbg,$scid,$qtr,$fields="g.id",$filter=NULL,$sem=0){	
	$dbo=PDBO;
	$condsem=($sem>0)? "AND (crs.semester=$sem || crs.semester=0 )":NULL;
...
$students=getHonorsStudentsLevel($db,$dbg,$lvl,$qtr,$fields,$cond);
$count=count($students);


/* 3 */
$fields="crs.id,crs.supsubject_id,crs.is_aggregate,crs.subject_id,crs.code,crs.is_primary";
$courses=getHonorsSubjectsLevel($db,$dbg,$lvl,$fields,$filter=NULL,$sem);	
$numcrs=count($courses);

/* 4 */
$numsub=getNumberOfCoursesLevel($db,$dbg,$lvl,$where="AND crs.supsubject_id>0",$sem);
$numpri=getNumberOfCoursesLevel($db,$dbg,$lvl,$where="AND crs.supsubject_id<1 AND crs.is_primary=1",$sem);

/* 5 */
$fields="g.id";
for($i=0;$i<$count;$i++){ $grades[$i]=honorsGradesLevel($db,$dbg,$students[$i]['scid'],$qtr,$fields,$filter=NULL,$sem); }	
...
<h5>
	Std Process Honors Level - <?php echo $cr['name']; ?> (<?php echo $count; ?>)
	| <?php $this->shovel('homelinks'); ?>
	| <a class="u" id="btnExport" >Excel</a> 	
	| <a href="<?php echo URL.'lir'; ?>" >L I R</a>
	| <span class="u" onclick="pclass('hd');" >Show</span>
	| <a href="<?php echo URL.'honors/reportLevel/'.$lvl; ?>" >Report</a>
	| <a href="<?php echo URL.'honors/certificatesByClassroom/'.$crid.DS.$sy.DS.$qtr; ?>" >Certificates</a>
	| <a onclick="pclass('id');" >ID</a>
<?php if($lvl>12): ?>	
	<?php if($qtr<3): ?>
		| <a href="<?php echo URL.'honors/processLevel/'.$lvl.'?sem=1'; ?>" >Sem1</a>
	<?php else: ?>
		| <a href="<?php echo URL.'honors/processLevel/'.$lvl.'?sem=2'; ?>" >Sem2</a>	
	<?php endif; ?>	
<?php endif; ?>	
...
<?php endfor; ?>
<td><?php echo ($is_qual)? 1:'-'; ?>
	<input type="hidden" name="posts[<?php echo $i; ?>][is_qualified]" value="<?php echo $is_qual; ?>"  />	
</td>
...
<td>
	<?php // $same=($honor==$dbhonor)? true:false; ?>
	<?php // if(!$same): ?>
		<input class="vc50 center" name="posts[<?php echo $i; ?>][honor]" value="<?php echo $honor; ?>"  />
		<input type="hidden" name="posts[<?php echo $i; ?>][scid]" value="<?php echo $scid; ?>"  />
	<?php // endif; ?>
</td>
<td><?php $honor_dg=getHonorByGenave($genave); echo ($is_qual)? $honor_dg:'-'; ?>
	<input class="vc50 center" type="hidden" name="posts[<?php echo $i; ?>][honor_dg]" value="<?php echo ($is_qual)? $honor_dg:NULL; ?>"  />
</td>
<td><?php echo $students[$i]['dghonor']; ?></td>
...
<table class="gis-table-bordered table-altrow" style="font-size:0.8em;" >
	...
	<tr><td>1: Genave 87 above. (Summarizer)</td></tr>
