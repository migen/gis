...
$q="SELECT `q{$qtr}_days_total` AS numdays FROM {$dbg}.05_attendance_months WHERE level_id='$lvl' LIMIT 1; ";
$sth=$db->querysoc($q);
$attmonths=$sth->fetch();
$numdays=$attmonths['numdays'];
...
$q="
	SELECT g.id AS gid,c.name AS student,summ.scid,c.code AS studcode,
		...
		summ.conduct_q{$qtr} AS summ_conduct,aw.`is_conduct_awardee_q{$qtr}` AS `is_awardee`,sumx.skip_q{$qtr} AS skip
...
if($lvl>9){	/* gr7-g12 */
	if($ec){
		function adjustConduct($conduct,$tardy,$minor,$major_a,$major_b,$absents=0,$cutoff_adee){			
			/* 1 is_conduct_awardee */
			$is_awardee=0;
			if($conduct>=$cutoff_adee && $absents<5 && $tardy==0 && $minor==0 && $major_a==0 && $major_b==0){ $is_awardee=1; }
			/* 2 tardy */
			switch($tardy){
			...
			/* 3 minor */
			if($minor>5){ $conduct=($conduct>75)? 75:$conduct; }
			elseif($minor>4){ $conduct=($conduct>80)? 80:$conduct; }
...
<?php if($edit): ?>
	...
	<tr class="center" >
		...
		<?php $absents=$numdays-$rows[$i]['present']; ?>		
...
<td>
		<?php 
			$res=adjustConduct($rows[$i]['conduct'],$rows[$i]['tardy'],$rows[$i]['minor'],$rows[$i]['major_a'],$rows[$i]['major_b'],$absents,$cutoff_adee);
			// pr($res);
...
<tr><td>Absc</td><td>Absences</td></tr>
