		...
		<select id="icri" class="full" >	
			...
			<?php foreach($data['selects']['criteria'] as $sel): ?>
				<option value="<?php echo $sel['id']; ?>"> 
					<?php echo $sel['crstype_id'].' - '.$sel['name'].' - #'.$sel['id']; ?> </option>
			<?php endforeach; ?>
		</select>				
...
<tr>
	...
	<td>
		<select id="sub<?php echo $i; ?>" class="sub vc100" name='components[<?php echo $i; ?>][subject_id]'  >
			<?php	foreach($data['selects']['subjects'] as $sel): ?>
				<option value="<?php echo $sel['id']; ?>"><?php echo $sel['name']; ?></option><?php	endforeach; ?>				
		</select>
	</td>	
	...
	<td>
		<select id="cri<?php echo $i; ?>" class="cri vc200" name='components[<?php echo $i; ?>][criteria_id]'  >
			<?php	foreach($data['selects']['criteria'] as $sel): ?><option value="<?php echo $sel['id']; ?>">
				<?php echo $sel['crstype_id'].' - '.$sel['name']; ?></option><?php	endforeach; ?>				
		</select>
	</td>
...
<p>
	...
	<button><a href="<?php echo URL.'mis/components/'.$level_id; ?>" class="no-underline" >Cancel</a></button>
</p>
