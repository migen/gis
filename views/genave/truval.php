...
<h5>
	True Values (Truval) - General Average (<?=$num_students;?>)
...
?>
	...
	<?php $qqtr = 'q'.$j; ?>
		...
		<?php endfor; ?>
			...
			<td class="final <?php echo ($rdbga!=$rdtg)? 'bg-salmon':null; ?>">
				<input class="vc50 center" name="sum[<?php echo $g; ?>][ave_q<?php echo $j; ?>]" value="<?php echo $rdtg; ?>"  readonly />
				<!-- add q1t to fgt -->
				<?php if(!$sem): ?>
					<?php $ftg += $rdtg; ?>				
				<?php else: ?>
...
<!-- ============================== for FG ROW not column ============================== -->
	...
	<?php for($i=0;$i<$num_subjects;$i++): ?> <!-- subject iterator -->			
			<?php 				
				...
				// $score=($row[$i]['q1']+$row[$i]['q2']+$row[$i]['q3']+$row[$i]['q4'])/4;
				$score=(number_format($row[$i]['q1'],$decicard)+number_format($row[$i]['q2'],$decicard)+number_format($row[$i]['q3'],$decicard)+number_format($row[$i]['q4'],$decicard))/4;
