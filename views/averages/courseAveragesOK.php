...
$deciave=isset($_GET['deciave'])?$_GET['deciave']:$deptdeci;
...
<?php if($course['with_scores']==1): ?>
	| <a href='<?php echo URL."teachers/scores/$course_id/$sy/$qtr"; ?>' >Class Records</a>
<?php else: ?>	
	<?php if($course['is_num']==1): ?>
		| <a href='<?php echo URL."teachers/grades/$course_id/$sy/$qtr"; ?>' >Class Records (G)</a>	
	<?php else: ?>	
		| <a href='<?php echo URL."grades/dg/$course_id/$sy/$qtr"; ?>' >Class Records (DG)</a>		
	<?php endif; ?>	
<?php endif; ?>	
...
<?php else: ?>
	| <a href='<?php echo URL."averages/course/$course_id/$sy/$qtr?sort=c.position"; ?>' >Position</a> 		
<?php endif; ?>
...
<form method="GET" >
	...
	Deciave <input value="<?php echo (isset($_GET['deciave']))? $_GET['deciave']:$deptdeci; ?>"
...
function redirCtype(){
	var vurl = gurl+"/averages/course/"+crs+ds+sy+ds+qtr+'?ctype='+$('#ctype').val()+'&dept='+$('#dept').val();
...
<tr><th class="bg-blue2" >Status</th><td><?php echo ucfirst('Q'.$qtr); ?> - <?php echo ($is_locked)? 'Closed' : 'Open'; ?></td></tr>
</table>
</p>

<?php if(!$is_locked): ?>
	<h5 class="brown" >Please <span class="u" >Finalize.</span></h5>
<?php else: ?>
	<?php if($qtr==$sqtr): ?>
		<h5 class="brown" ><span class="" >*Re-Finalize if any red box Ave (column) exists.</span></h5>	
	<?php endif; ?>
<?php endif; ?>
...
<?php if(!$same): ?>
	...
	<br /><input class="vc50 center" type="<?php echo $intype; ?>" name="grades[<?php echo $i; ?>][dg]" value="<?php echo $dg; ?>" readonly />	
...
<?php if($admin): ?>
	...
	<input type="submit" name="submit" value="Finalize"  /></p>
<?php endif; ?>

<?php if($qtr==$_SESSION['qtr']): ?>
	<?php if($_SESSION['user']['role_id']==RTEAC): ?>
		<?php if(!$is_locked): ?>			
			<br /><input type="submit" name="submit" value="Finalize"  />
		<?php else: ?>
			<?php if(!$updated): ?>		
...
<?php else: ?>
	<?php if(!$is_locked): ?>
		...
		<br /><h4 class="brown" >Not present quarter!</h4><input type="submit" name="submit" value="Finalize"  />
	<?php endif; ?>
